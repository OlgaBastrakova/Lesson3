...
class PDFExtractor:
    ...
    def put_pdf_into_file (self, path_katalog, pages_by_kod_SP,date_of_file_result):
        print("ghbdtn")
        for kod_SP, pdf_writer in pages_by_kod_SP.items():
            num_pages_in_pdf = len(pdf_writer.pages)
            output_file_name = os.path.join(path_katalog, f"{date_of_file_result}_{kod_SP}_{num_pages_in_pdf}.pdf")
...
def main():
    ...
    pages_by_kod_SP = pdf_extractor.split_pdf_by_payment_number(payment_number_to_code_result, path_katalog, date_of_file_result)
    pdf_extractor.put_pdf_into_file(path_katalog, pages_by_kod_SP, date_of_file_result)
